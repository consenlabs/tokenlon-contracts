...
import "./utils/MultiSigLibEIP712.sol";
...
contract MultiSig is MultiSigLibEIP712 {
     ...
     */
    constructor(address[] memory _owners, uint256 _required) public validRequirement(_owners.length, _required) MultiSigLibEIP712() {
    ...
    function submitTransaction(
        ...
        bytes32 EIP712SignDigest = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encode(SUBMIT_TRANSACTION_TYPE_HASH, transactionId, destination, value, data))
            )
        );
    ...
    function confirmTransactionBySigner(address signer, uint256 transactionId) internal transactionExists(transactionId) notConfirmed(transactionId, signer) {
        // Confirm
    ...
    function executeTransactionBySigner(address signer, uint256 transactionId) internal notExecuted(transactionId) {
    ...
    function confirmTransaction(
        ...
        uint256 transactionId,
        bytes memory sig
    ) public virtual ownerExists(signer) transactionExists(transactionId) notConfirmed(transactionId, signer) {
        // CONFIRM_TRANSACTION_TYPE_HASH = keccak256("confirmTransaction(uint256 transactionId)");
        bytes32 EIP712SignDigest = keccak256(
            abi.encodePacked(bytes1(0x19), bytes1(0x01), EIP712_DOMAIN_SEPARATOR, keccak256(abi.encode(CONFIRM_TRANSACTION_TYPE_HASH, transactionId)))
        );
        validateSignature(signer, EIP712SignDigest, sig);
