...
contract SpenderTest is BalanceUtil {
    ...
    function setUp() public {
        // Deploy
        spender = new Spender(trusted);

        // Set user's mock tokens balance and approve
        setEOABalanceAndApprove(user, address(spender), tokens, 100);
    ...
    function testCannotSpendFromUserByNotAuthorized() public {
        ...
        spender.spendFromUserTo(user, address(mockERC20), recipient, 100);
    }
    ...
    function testSpendFromUserTo() public {
        ...
        uint256 amount = 100;
        vm.prank(authorized);
        spender.spendFromUserTo(user, address(mockERC20), recipient, amount);
    ...
    function testSpendFromUserToWithNoReturnValueToken() public {
        ...
        uint256 amount = 100;
        vm.prank(authorized);
        spender.spendFromUserTo(user, address(noReturnERC20), recipient, amount);
